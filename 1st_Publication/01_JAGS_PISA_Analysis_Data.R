...
pred_grid <- Pisa_Europe_Data |>
  ...
  mutate(year_orig = year-baseline_year) |>
  mutate(Country_num = as.numeric(as.factor(Country)),
...
# For the independent model, recall that we excluded countries with one data points, 
#the grid should only include the countries used in fitting the model.
# Grid for independent model, by creating pred_grid for the SCountry
