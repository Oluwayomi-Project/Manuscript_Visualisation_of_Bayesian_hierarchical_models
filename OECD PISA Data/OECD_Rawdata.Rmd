...
```{r}
if (!dir.exists(here::here("Saved_PISA_Data")))
...
# include the ISO code to the 2009 and 2012 data.
...
Math_2012 <- DATA_2012 |> 
  select(iso3c, Mean, s.e.) |>
  mutate(year = 2012) |>
  rename(math = Mean, sigma = s.e., CNT = iso3c) |>
  as_tibble()
```
...
*The UNSD csv file has 249 countries and the PISA data has 107 countries. Hence, left-joining the two data sets we still have a total of 107 countries with their region information. These avoided the loss of lose any data*.
...
PISA_Data <- PISA_Data |>
  select(Country, Continent, Region, Income, year, year_orig, math, sigma)


save(PISA_Data, file =pisaFileMaths)
```

*The original PISA data set has 462 observations, By merging the two data sets, we have 444 observations, 91 countries in total with 8 variables - year, math score, sigma, year_orig, country, continent, region, and income. Some ISO code does not have a valid name in the WDI package we used to give the country names.*
