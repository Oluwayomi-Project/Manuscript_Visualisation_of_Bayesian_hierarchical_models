...
ggplot(data = Combined_Int_Estimates)+
  ...
  theme(axis.text.x = element_blank(),
      legend.position = "bottom",strip.background = element_rect(fill = Region_color_flag))
...
ggplot(data = Combined_Int_Estimates)+
  ...
  theme_bw()+
  theme(axis.text.x = element_blank(),
        strip.background.x = element_rect(fill = Region_color_flag),
      legend.position = "bottom")
...
ggplot(data = Combined_Slop_Estimates)+
  ...
  theme_bw()+
  theme(axis.text.x = element_blank(),
        strip.background = element_text(fill =Region_color_flag),
      legend.position = "bottom")
```
...
All countries with one data point, and countries where the hierarchical structure downweight its estimate.

Countries like; Albania, Belarus, Ukraine, Moldova, Macedonia,
Netherlands, Luxembourg, Liechtenstein, Czechia, Switzerland.

We want to investigate 10 countries.

The first thing to do is to filter out the regression line on the data across the different fitted model for these countries.
